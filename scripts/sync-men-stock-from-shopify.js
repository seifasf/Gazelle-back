/**
 * Pull live Shopify available qty for Men's products → OMS realStock,
 * then release Out-of-stock orders that are now fully covered.
 *
 * Usage:
 *   DRY_RUN=1 node scripts/sync-men-stock-from-shopify.js
 *   node scripts/sync-men-stock-from-shopify.js
 */
...
function isMenShopifyProduct(sp) {
  const type = String(sp.productType || '');
  if (/^men\b/i.test(type)) return true;
  const tags = (sp.tagsList || sp.tags || []).map((t) => String(t));
  const hasMenTag = tags.some((t) => /^men$/i.test(t.trim()));
  if (hasMenTag && !/^women\b/i.test(type)) return true;
  if (/^men\b/i.test(String(sp.title || ''))) return true;
  return false;
}
...
async function run() {
  ...
  const menShopify = shopifyProducts.filter(
    (sp) => sp.status === 'ACTIVE' && isMenShopifyProduct(sp)
  );
  console.log(
    `Shopify active products: ${shopifyProducts.filter((p) => p.status === 'ACTIVE').length}`
  );
  console.log(`Men products (Shopify): ${menShopify.length}`);

  const items = [];
  const unmatched = [];
  let shopifyUnits = 0;

  for (const sp of menShopify) {
  ...
  console.log(`Shopify men units (sum available): ${shopifyUnits}`);
  ...
  const sweep = await releaseOutOfStockOrdersIfRestocked(allVariantIds, {
    actorUserId: null,
    note: 'Auto: men Shopify stock sync — back to Ready to ship',
  });
  ...
  console.log('\n=== DONE ===');
  console.log(`Men Shopify products: ${menShopify.length}`);
  ...
  const menShopifyIds = menShopify.map((p) => p.id);
  await Product.updateMany(
    { shopifyProductId: { $in: menShopifyIds } },
